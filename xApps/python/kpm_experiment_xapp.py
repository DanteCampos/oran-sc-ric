...
class MyXapp(xAppBase):
    def __init__(self, config, http_server_port, rmr_port, metrics, ue_ids, target_thr):
        super(MyXapp, self).__init__(config, http_server_port, rmr_port)
        self.kpm_data = pandas.DataFrame(columns=["ue_id", "timestamp", "prb_alloc", "target_thr"] + metrics)
        self.target_thr = target_thr
        # Initializing the SDL entries for PRB allocation per UE
    ...
    def my_subscription_callback(self, e2_agent_id, subscription_id, indication_hdr, indication_msg, kpm_report_style, ue_id):
        ...
        if kpm_report_style != 5:
            print("KPM Report Style {} not supported. Only 5 is supported.".format(kpm_report_style))
        else:
            data = {
                'timestamp': str(indication_hdr['colletStartTime']),
                'target_thr': self.target_thr
            }
            for ue_id, ue_meas_data in meas_data["ueMeasData"].items():
                print("--UE_id: {}".format(ue_id))
                data['prb_alloc'] = self.sdl.get(ns="prb_alloc", key=str(ue_id))
                print("--PRB Allocation from SDL: {}".format(data['prb_alloc']))
                for metric_name, value in ue_meas_data["measData"].items():
                    data[metric_name] = value
                    print("---Metric: {}, Value: {}".format(metric_name, value))
                self.sdl.set(ns="metrics", key=str(ue_id), value=data)
                data['ue_id'] = ue_id
                new_row = pandas.DataFrame([data])
                self.kpm_data = pandas.concat([self.kpm_data, new_row], ignore_index=True)
    ...
    # HTTP GET handler to send collected KPM data
    def send_collected_data(self, name:str, path:str, data:bytes, ctype:str):
        response = ricrest.initResponse(status=200,response="KPM data")
        response['payload'] = self.kpm_data.to_csv(index=False)
        return response
...
if __name__ == '__main__':
    ...
    parser.add_argument("--target_thr", type=str, default='10000', help="UE target throughput in kbps")
    ...
    target_thr = float(args.target_thr)

    # Create MyXapp.
    myXapp = MyXapp(config, args.http_server_port, args.rmr_port, metrics, ue_ids, target_thr)
    ...
    myXapp.httpServer.handler.add_handler(myXapp.httpServer.handler, method="GET", name="kpmdata", uri="/data", callback=myXapp.send_collected_data)
